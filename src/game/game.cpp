...
static int WriteMapPresentation(const std::filesystem::path &map_filepath, CMap &map)
{
	std::filesystem::path old_map_setup_filepath = map_filepath;
	old_map_setup_filepath.replace_extension(".smp");
	if (std::filesystem::exists(old_map_setup_filepath)) {
		std::filesystem::remove(old_map_setup_filepath);
	}

	gsml_data data;

	if (!map.Info->get_name().empty()) {
		data.add_property("name", map.Info->get_name());
	}
	data.add_child(gsml_data::from_size(map.Info->get_map_size(), "map_size"));

	int numplayers = 0;
	int topplayer = PlayerMax - 2;

	gsml_data player_types_data("player_types");
	...
	for (int i = 0; i <= topplayer; ++i) {
		...
		player_types_data.add_value(player_type_to_string(player_type));
		if (player_type == player_type::person) {
			++numplayers;
		}
	}

	data.add_child(std::move(player_types_data));

	if (map.Info->get_presets() != nullptr) {
		data.add_property("settings", map.Info->get_presets()->get_identifier());
	}
...
*/
int WriteMapSetup(const std::filesystem::path &map_setup_filepath, CMap &map, const int writeTerrain)
{
	std::unique_ptr<FileWriter> f;

	try {
		f = CreateFileWriter(path::to_string(map_setup_filepath));
	...
	//					f->printf("SetTile(%3d, %d, %d, %d)\n", n, j, i, value);
		...
		log::log_error("Can't save map setup: \"" + path::to_string(map_setup_filepath) + "\".");
		return -1;
	}
...
**  @param writeTerrain   write the tiles map in the .sms
*/
int SaveStratagusMap(const std::filesystem::path &map_filepath, CMap &map, const int writeTerrain)
{
	if (!map.Info->get_map_width() || !map.Info->get_map_height()) {
		throw std::runtime_error(path::to_string(map_filepath) + ": invalid Stratagus map");
	}

	if (map_filepath.extension() != ".wmp") {
		throw std::runtime_error(path::to_string(map_filepath) + ": invalid Stratagus map filename");
	}

	if (WriteMapPresentation(map_filepath, map) == -1) {
		return -1;
	}

	std::filesystem::path map_setup_filepath = map_filepath;
	map_setup_filepath.replace_extension(".sms");

	return WriteMapSetup(map_setup_filepath, map, writeTerrain);
}
