...
public:
	...
	QString get_locale_qstring() const
	{
		return QString::fromStdString(this->get_locale());
	}

	Q_INVOKABLE void set_locale(const std::string &locale);

	void set_locale_qstring(const QString &locale)
	{
		this->set_locale(locale.toStdString());
	}
