...
//					CMap::Map.Fields = new CMapField[CMap::Map.Info.MapWidth * CMap::Map.Info.MapHeight];
				...
				//Wyrmgus end
					...
					const int subsubargs = lua_rawlen(l, -1);
					if (subsubargs != CMap::Map.Info.MapWidth * CMap::Map.Info.MapHeight) {
...
{
	LuaCheckArgs(l, 1);
	CMap::Map.BorderTerrain = CTerrainType::Get(LuaToString(l, 1));
...
void SetTileTerrain(const std::string &terrain_ident, const Vec2i &pos, int value, int z)
{
	...
	CTerrainType *terrain = CTerrainType::Get(terrain_ident);
...
static int CclSetMapTemplateTileTerrain(lua_State *l)
{
	...
	if (!terrain_ident.empty()) {
		terrain = CTerrainType::Get(terrain_ident);
		if (!terrain) {
...
static int CclSetMapTemplateResource(lua_State *l)
{
	...
	if (nargs >= 5) {
		unique = UniqueItem::Get(LuaToString(l, 5));
		if (!unique) {
			LuaError(l, "Unique item doesn't exist.\n");
		}
	}
	
	map_template->Resources[std::pair<int, int>(ipos.x, ipos.y)] = std::tuple<CUnitType *, int, UniqueItem *>(unittype, resources_held, unique);
...
static int CclSetMapTemplateUnit(lua_State *l)
{
	...
	if (nargs >= 7) {
		unique = UniqueItem::Get(LuaToString(l, 7));
		if (!unique) {
			LuaError(l, "Unique item doesn't exist.\n");
		}
	}
...
static int CclDefineTerrainType(lua_State *l)
{
	...
	CTerrainType *terrain = CTerrainType::GetOrAdd(terrain_ident);
	...
	for (lua_pushnil(l); lua_next(l, 2); lua_pop(l, 1)) {
		...
		if (!strcmp(value, "Name")) {
			...
			for (int j = 0; j < subargs; ++j) {
				CTerrainType *base_terrain = CTerrainType::GetOrAdd(LuaToString(l, -1, j + 1));
			...
			const int subargs = lua_rawlen(l, -1);
			for (int j = 0; j < subargs; ++j) {
				CTerrainType *border_terrain = CTerrainType::Get(LuaToString(l, -1, j + 1));
			...
			const int subargs = lua_rawlen(l, -1);
			for (int j = 0; j < subargs; ++j) {
				CTerrainType *overlay_terrain = CTerrainType::Get(LuaToString(l, -1, j + 1));
			...
			for (int j = 0; j < subargs; ++j) {
				...
				if (transition_terrain_name != "any") {
					transition_terrain = CTerrainType::Get(transition_terrain_name);
					
					if (transition_terrain == nullptr) {
						LuaError(l, "Terrain doesn't exist.");
					}
				}
				int transition_terrain_id = transition_terrain ? transition_terrain->GetIndex() : -1;
				++j;
...
static int CclDefineMapTemplate(lua_State *l)
{
	...
	for (lua_pushnil(l); lua_next(l, 2); lua_pop(l, 1)) {
		...
		if (!strcmp(value, "Name")) {
			...
			CTerrainType *terrain_type = CTerrainType::Get(LuaToString(l, -1));
		...
		if (!strcmp(value, "Name")) {
			...
			for (int j = 0; j < subargs; ++j) {
				...
				if (faction->GetCivilization()) {
					faction->GetCivilization()->Sites.push_back(site);
				}
			}
			...
			for (int j = 0; j < subargs; ++j) {
				...
				site->HistoricalBuildings.push_back(std::tuple<CDate, CDate, const UnitClass *, UniqueItem *, CFaction *>(start_date, end_date, building_class, unique, building_owner));
			}
			...
			for (int j = 0; j < subargs; ++j) {
				...
				site->HistoricalResources.push_back(std::tuple<CDate, CDate, CUnitType *, UniqueItem *, int>(start_date, end_date, unit_type, unique, quantity));
			}
...
static int CclDefineTerrainFeature(lua_State *l)
{
	...
	for (lua_pushnil(l); lua_next(l, 2); lua_pop(l, 1)) {
		...
		if (!strcmp(value, "Name")) {
			...
			CTerrainType *terrain = CTerrainType::Get(LuaToString(l, -1));
			if (!terrain) {
				LuaError(l, "Terrain doesn't exist.");
			}
...
static int CclGetSiteData(lua_State *l)
{
	...
	if (!strcmp(data, "Name")) {
		lua_pushstring(l, site->GetName().utf8().get_data());
		return 1;
