...
//Wyrmgus end
{
	...
	if (z == UI.CurrentMapLayer->ID) {
		...
		if (event_map_layer->Plane != nullptr && CMap::Map.GetCurrentPlane() != event_map_layer->Plane && UI.PlaneButtons[event_map_layer->Plane->GetIndex()].X != -1) {
			MinimapEvents[NumMinimapEvents].pos.x = UI.PlaneButtons[event_map_layer->Plane->GetIndex()].X + (UI.PlaneButtons[event_map_layer->Plane->GetIndex()].Style->Width / 2);
			MinimapEvents[NumMinimapEvents].pos.y = UI.PlaneButtons[event_map_layer->Plane->GetIndex()].Y + (UI.PlaneButtons[event_map_layer->Plane->GetIndex()].Style->Height / 2);
		} else if (event_map_layer->World != nullptr && CMap::Map.GetCurrentWorld() != event_map_layer->World && UI.WorldButtons[event_map_layer->World->GetIndex()].X != -1) {
			MinimapEvents[NumMinimapEvents].pos.x = UI.WorldButtons[event_map_layer->World->GetIndex()].X + (UI.WorldButtons[event_map_layer->World->GetIndex()].Style->Width / 2);
			MinimapEvents[NumMinimapEvents].pos.y = UI.WorldButtons[event_map_layer->World->GetIndex()].Y + (UI.WorldButtons[event_map_layer->World->GetIndex()].Style->Height / 2);
