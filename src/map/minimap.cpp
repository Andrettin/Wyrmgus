...
//Wyrmgus end
{
	...
	if (z == UI.CurrentMapLayer->ID) {
		...
		if (event_map_layer->Plane != nullptr && CMap::Map.GetCurrentPlane() != event_map_layer->Plane && UI.PlaneButtons[event_map_layer->Plane->Index].X != -1) {
			MinimapEvents[NumMinimapEvents].pos.x = UI.PlaneButtons[event_map_layer->Plane->Index].X + (UI.PlaneButtons[event_map_layer->Plane->Index].Style->Width / 2);
			MinimapEvents[NumMinimapEvents].pos.y = UI.PlaneButtons[event_map_layer->Plane->Index].Y + (UI.PlaneButtons[event_map_layer->Plane->Index].Style->Height / 2);
		} else if (event_map_layer->World != nullptr && CMap::Map.GetCurrentWorld() != event_map_layer->World && UI.WorldButtons[event_map_layer->World->Index].X != -1) {
			MinimapEvents[NumMinimapEvents].pos.x = UI.WorldButtons[event_map_layer->World->Index].X + (UI.WorldButtons[event_map_layer->World->Index].Style->Width / 2);
			MinimapEvents[NumMinimapEvents].pos.y = UI.WorldButtons[event_map_layer->World->Index].Y + (UI.WorldButtons[event_map_layer->World->Index].Style->Height / 2);
