...
static void DrawUnitInfo_Training(const CUnit &unit)
{
	if (unit.Orders.size() == 1 || unit.Orders[1]->Action != UnitActionTrain) {
		...
		if (UI.SingleTrainingButton) {
			...
			//Wyrmgus sta
			UnitTypeVariation *variation = order.GetUnitType().GetDefaultVariation(CPlayer::GetThisPlayer());
//			const CIcon *icon = order.GetUnitType().Icon.Icon;
			const CIcon *icon = (variation != nullptr && variation->GetIcon() != nullptr) ? variation->GetIcon() : order.GetUnitType().GetIcon();
			//Wyrmgus end
			//Wyrmgus start
		...
		if (!UI.TrainingButtons.empty()) {
			...
			for (size_t i = 0; i < unit.Orders.size(); ++i) {
				if (unit.Orders[i]->Action == UnitActionTrain) {
					...
					UnitTypeVariation *variation = order.GetUnitType().GetDefaultVariation(CPlayer::GetThisPlayer());
					const CIcon *icon = (variation && variation->GetIcon()) ? variation->GetIcon() : order.GetUnitType().GetIcon();
					//Wyrmgus start
...
void DrawTime() {
	...
	if (CPlayer::GetThisPlayer()) {
		CCalendar *calendar = CCivilization::Get(CPlayer::GetThisPlayer()->Race)->GetCalendar();
	...
	for (size_t i = 0; i < UI.PlaneButtons.size(); ++i) {
		if (UI.PlaneButtons[i].X != -1) {
			DrawUIButton(UI.PlaneButtons[i].Style,
				...
				| ((UI.PlaneButtons[i].Clicked || CMap::Map.GetCurrentPlane() == CPlane::Get(i)) ? MI_FLAGS_CLICKED : 0),
	...
	for (size_t i = 0; i < UI.WorldButtons.size(); ++i) {
		if (UI.WorldButtons[i].X != -1) {
			DrawUIButton(UI.WorldButtons[i].Style,
				...
				| ((UI.WorldButtons[i].Clicked || CMap::Map.GetCurrentWorld() == CWorld::Get(i)) ? MI_FLAGS_CLICKED : 0),
...
constexpr int MESSAGES_MAX = 10;			/// How many can be displayed
//Wyrmgus start
constexpr int OBJECTIVES_MAX = 32;			/// How many objectives be displayed
//Wyrmgus end
