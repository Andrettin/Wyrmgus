...
//Wyrmgus end
{
	...
	if ((1 << button) == LeftButton && GrandStrategy && !GameRunning && GameResult == GameNoResult && !GrandStrategyGamePaused) {
		...
		} else if (GrandStrategyGame.SelectedProvince != -1 && !(MouseButtons & LeftButton) && UI.GrandStrategyShowRulerButton.Clicked && (GrandStrategyInterfaceState == "town-hall" || GrandStrategyInterfaceState == "stronghold") && UI.GrandStrategyShowRulerButton.X != -1 && GrandStrategyGame.PlayerFaction != NULL && GrandStrategyGame.PlayerFaction->Ministers[CharacterTitleHeadOfState] != NULL) {
...
static void MenuHandleKeyUp(unsigned key, unsigned keychar)
{
	...
	*/
	...
	if (GrandStrategy && !GameRunning && GameResult == GameNoResult && !GrandStrategyGamePaused) {// if in grand strategy mode
		...
		} else if (key == 'r' && GrandStrategyGame.SelectedProvince != -1 && UI.GrandStrategyShowRulerButton.X != -1 && GrandStrategyGame.PlayerFaction != NULL && GrandStrategyGame.PlayerFaction->Ministers[CharacterTitleHeadOfState] != NULL && UI.GrandStrategyShowRulerButton.HotkeyPressed && (GrandStrategyInterfaceState == "town-hall" || GrandStrategyInterfaceState == "stronghold")) {
