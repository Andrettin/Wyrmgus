		...
		if (unit.RallyPointPos.x != -1 && unit.RallyPointPos.y != -1 && newUnit->CanMove()) {
			...
			for (size_t j = 0; j != table.size(); ++j) {
				...
				if (newUnit->Type->RepairRange && table[j]->Type->RepairHP && table[j]->Variable[HP_INDEX].Value < table[j]->GetModifiedVariable(HP_INDEX, VariableMax) && (table[j]->Player == newUnit->Player || newUnit->IsAllied(*table[j]))) { //see if can repair
					...
					newUnit->Type->BoolFlag[HARVESTER_INDEX].value && table[j]->GivesResource && newUnit->Type->ResInfo[table[j]->GivesResource] && table[j]->Type->BoolFlag[CANHARVEST_INDEX].value && (table[j]->GivesResource == TradeCost || table[j]->Player == newUnit->Player || (table[j]->Player->IsAllied(*newUnit->Player) && newUnit->Player->IsAllied(*table[j]->Player)) || table[j]->Player->Index == PlayerNumNeutral) && (table[j]->GivesResource != TradeCost || table[j]->Player != newUnit->Player)
				) { // see if can harvest
