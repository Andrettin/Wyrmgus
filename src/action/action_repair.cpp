...
QPoint COrder_Repair::get_shown_target_pos(const CViewport &vp) const
{
	if (this->get_reparable_target() != nullptr) {
		return vp.scaled_map_to_screen_pixel_pos(this->get_reparable_target()->get_scaled_map_pixel_pos_center());
	} else {
		return vp.TilePosToScreen_Center(this->goalPos);
	}
}

QColor COrder_Repair::get_shown_target_color() const
{
...
{
	UnitShowAnimation(unit, unit.get_animation_set()->Repair);
}
