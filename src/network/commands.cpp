...
//			CommandMove(unit, pos, status);
		...
		case MessageCommandBuild:
			CommandLog("build", &unit, status, pos.x, pos.y, NoUnitP, CUnitType::Get(dstnr)->Ident.c_str(), -1);
			CommandBuildBuilding(unit, pos, *CUnitType::Get(dstnr), status);
			break;
		...
		case MessageCommandTrain:
			//Wyrmgus start
//			CommandLog("train", &unit, status, -1, -1, NoUnitP, CUnitType::Get(dstnr)->Ident.c_str(), -1);
//			CommandTrainUnit(unit, *CUnitType::Get(dstnr), status);
			CommandLog("train", &unit, status, -1, -1, NoUnitP, CUnitType::Get(dstnr)->Ident.c_str(), arg1); // use X as a way to mark the player
			CommandTrainUnit(unit, *CUnitType::Get(dstnr), arg1, status);
			//Wyrmgus end
			break;
		case MessageCommandCancelTrain:
			...
			if (dstnr != (unsigned short)0xFFFF) {
				CommandLog("cancel-train", &unit, FlushCommands, -1, -1, NoUnitP,
						   CUnitType::Get(dstnr)->Ident.c_str(), (short)x);
				CommandCancelTraining(unit, (short)x, CUnitType::Get(dstnr));
			} else {
		...
		case MessageCommandUpgrade:
			...
			CommandLog("upgrade-to", &unit, status, -1, -1, NoUnitP,
					   CUnitType::Get(dstnr)->Ident.c_str(), -1);
			CommandUpgradeTo(unit, *CUnitType::Get(dstnr), status);
			break;
			*/
			if (arg1 == 2) { //use X as a way to mark whether this is an upgrade or a transformation
				CommandLog("transform-into", &unit, status, -1, -1, NoUnitP,
						   CUnitType::Get(dstnr)->Ident.c_str(), -1);
				CommandTransformIntoType(unit, *CUnitType::Get(dstnr));
			} else {
				CommandLog("upgrade-to", &unit, status, -1, -1, NoUnitP,
						   CUnitType::Get(dstnr)->Ident.c_str(), -1);
				CommandUpgradeTo(unit, *CUnitType::Get(dstnr), status);
