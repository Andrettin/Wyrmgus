...
void CGrandStrategyGame::CreateWork(const CUpgrade *work, CGrandStrategyHero *author, CGrandStrategyProvince *province)
{
	...
	if (work != nullptr) {
		work_name = work->GetName().utf8().get_data();
	} else {
...
void CGrandStrategyProvince::SetOwner(int civilization_id, int faction_id)
{
	for (const CUnitType *unit_type : CUnitType::GetAll()) { //change the province's military score to be appropriate for the new faction's technologies
		if (IsMilitaryUnit(*unit_type)) {
			...
			int new_owner_military_score_bonus = (faction_id != -1 ? GrandStrategyGame.Factions[civilization_id][faction_id]->MilitaryScoreBonus[unit_type->GetIndex()] : 0);
			if (old_owner_military_score_bonus != new_owner_military_score_bonus) {
				this->MilitaryScore += this->Units[unit_type->GetIndex()] * (new_owner_military_score_bonus - old_owner_military_score_bonus);
...
void CGrandStrategyProvince::SetSettlementBuilding(int building_id, bool has_settlement_building)
{
	...
	if (CUnitType::Get(building_id)->GetCivilization() != nullptr) {
		if (this->GetClassUnitType(CUnitType::Get(building_id)->GetClass()) != building_id && this->GetClassUnitType(CUnitType::Get(building_id)->GetClass()) != -1) {
			building_id = this->GetClassUnitType(CUnitType::Get(building_id)->GetClass());
		}
	}
	...
	for (int i = 0; i < MaxCosts; ++i) {
		if (CUnitType::Get(building_id)->GrandStrategyProductionEfficiencyModifier[i] != 0) {
			this->ProductionEfficiencyModifier[i] += CUnitType::Get(building_id)->GrandStrategyProductionEfficiencyModifier[i] * change;
		}
	}
	
	if (CUnitType::Get(building_id)->GetClass() != nullptr && CUnitType::Get(building_id)->GetClass()->Ident == "stronghold") { //increase the military score of the province, if this building is a stronghold
...
//	fprintf(stderr, "Setting the quantity of unit type \"%s\" to %d in the %s province.\n", CUnitType::Get(unit_type_id)->Ident.c_str(), quantity, this->Name.c_str());
	...
	if (IsMilitaryUnit(*CUnitType::Get(unit_type_id))) {
		this->MilitaryScore += change * (CUnitType::Get(unit_type_id)->DefaultStat.Variables[POINTS_INDEX].Value + (this->Owner != nullptr ? this->Owner->MilitaryScoreBonus[unit_type_id] : 0));
		this->OffensiveMilitaryScore += change * (CUnitType::Get(unit_type_id)->DefaultStat.Variables[POINTS_INDEX].Value + (this->Owner != nullptr ? this->Owner->MilitaryScoreBonus[unit_type_id] : 0));
	}
	
	if (CUnitType::Get(unit_type_id)->GetClass() != nullptr && CUnitType::Get(unit_type_id)->GetClass()->Ident == "worker") {
...
void CGrandStrategyProvince::SetPopulation(int quantity)
{
	...
	int worker_unit_type = this->GetClassUnitType(UnitClass::Get("worker"));
...
bool CGrandStrategyProvince::HasBuildingClass(std::string building_class_name)
{
	...
	int building_type = this->GetClassUnitType(UnitClass::Get(building_class_name));
...
CGrandStrategyHero *CGrandStrategyProvince::GetRandomAuthor()
{
	...
	for (size_t i = 0; i < this->Heroes.size(); ++i) {
		if (this->Heroes[i]->UnitType->GetClass() != nullptr && this->Heroes[i]->UnitType->GetClass()->Ident == "priest") { // first see if there are any heroes in the province from a unit class likely to produce scholarly works
...
void CGrandStrategyFaction::MinisterSuccession(int title)
{
	if (
		...
		for (size_t i = 0; i < this->Ministers[title]->Children.size(); ++i) {
			if (this->Ministers[title]->Children[i]->IsAlive() && this->Ministers[title]->Children[i]->IsVisible() && this->Ministers[title]->Children[i]->GetGender()->GetIdent() == "male") { //historically males have generally been given priority in throne inheritance (if not exclusivity), specially in the cultures currently playable in the game
		...
		for (size_t i = 0; i < this->Ministers[title]->Siblings.size(); ++i) { // now check for male siblings of the current ruler
			if (this->Ministers[title]->Siblings[i]->IsAlive() && this->Ministers[title]->Siblings[i]->IsVisible() && this->Ministers[title]->Siblings[i]->GetGender()->GetIdent() == "male") {
...
{
	return this->UnitType->DefaultStat.Variables[GENDER_INDEX].Value == 0 || (this->GetGender()->GetIndex() + 1) == this->UnitType->DefaultStat.Variables[GENDER_INDEX].Value; // hero not visible if their unit type has a set gender which is different from the hero's (this is because of instances where i.e. females have a unit type that only has male portraits)
}
...
{
	if (this->GetFaction()->GovernmentType == GovernmentTypeMonarchy && title == CharacterTitleHeadOfState && this->UnitType->GetClass() != nullptr && this->UnitType->GetClass()->Ident == "worker") { // commoners cannot become monarchs
		return false;
	} else if (this->GetFaction()->GovernmentType == GovernmentTypeTheocracy && title == CharacterTitleHeadOfState && this->UnitType->GetClass() != nullptr && this->UnitType->GetClass()->Ident != "priest") { // non-priests cannot rule theocracies
		return false;
	}
...
int CGrandStrategyHero::GetTitleScore(int title, CGrandStrategyProvince *province)
{
	...
	if (this->Civilization->GetIndex() != this->GetFaction()->Civilization) {
...
{
//	fprintf(stderr, "Triggering event \"%s\" for faction %s.\n", this->Name.c_str(), CFaction::Get(faction->Faction)->Name.c_str());	
	
	CclCommand("EventFaction = GetFactionFromName(\"" + std::string(CFaction::Get(faction->Faction)->GetIdent().utf8().get_data()) + "\");");
...
void AddProvinceClaim(std::string province_name, std::string civilization_name, std::string faction_name)
{
	...
	if (province_id != -1 && GrandStrategyGame.Provinces[province_id]) {
		...
		if (civilization) {
			int faction = CFaction::GetFactionIndex(faction_name);
			if (faction != -1) {
				GrandStrategyGame.Provinces[province_id]->AddFactionClaim(civilization->GetIndex(), faction);
			} else {
...
void RemoveProvinceClaim(std::string province_name, std::string civilization_name, std::string faction_name)
{
	...
	if (province_id != -1 && GrandStrategyGame.Provinces[province_id]) {
		...
		if (civilization) {
			int faction = CFaction::GetFactionIndex(faction_name);
			if (faction != -1) {
				GrandStrategyGame.Provinces[province_id]->RemoveFactionClaim(civilization->GetIndex(), faction);
			}
		}
	}
}
...
{
	int province = GetProvinceId(province_name);
	CCivilization *civilization = CCivilization::Get(faction_civilization_name);
	int faction = CFaction::GetFactionIndex(faction_name);
	...
	return GrandStrategyGame.Provinces[province]->BordersFaction(civilization->GetIndex(), faction);
}
...
std::string GetProvinceCivilization(std::string province_name)
{
	...
	if (GrandStrategyGame.Provinces[province_id]->Civilization != -1) {
		return CCivilization::Get(GrandStrategyGame.Provinces[province_id]->Civilization)->GetIdent().utf8().get_data();
	} else {
		return "";
	}
}
...
{
	int province_id = GetProvinceId(province_name);
	const CUnitType *building_type = CUnitType::Get(building_ident);
	
	return GrandStrategyGame.Provinces[province_id]->SettlementBuildings[building_type->GetIndex()];
}
...
int GetProvinceUnitQuantity(std::string province_name, std::string unit_type_ident)
{
	...
	return GrandStrategyGame.Provinces[province_id]->Units[unit_type->GetIndex()];
}
...
std::string GetProvinceOwner(std::string province_name)
{
	...
	return CFaction::Get(GrandStrategyGame.Provinces[province_id]->Owner->Faction)->GetIdent().utf8().get_data();
}
...
void SetFactionGovernmentType(std::string civilization_name, std::string faction_name, std::string government_type_name)
{
	...
	if (civilization) {
		int faction = CFaction::GetFactionIndex(faction_name);
		if (faction != -1) {
			GrandStrategyGame.Factions[civilization->GetIndex()][faction]->GovernmentType = government_type_id;
		}
	}
}
...
{
	CCivilization *civilization = CCivilization::Get(civilization_name);
	CCivilization *second_civilization = CCivilization::Get(second_civilization_name);
	
	int diplomacy_state_id = GetDiplomacyStateIdByName(diplomacy_state_name);
	
	if (civilization && second_civilization) {
		int faction = CFaction::GetFactionIndex(faction_name);
		int second_faction = CFaction::GetFactionIndex(second_faction_name);
		if (faction != -1 && second_faction != -1) {
			GrandStrategyGame.Factions[civilization->GetIndex()][faction]->DiplomacyStateProposals[GrandStrategyGame.Factions[second_civilization->GetIndex()][second_faction]] = diplomacy_state_id;
		}
	}
}
...
std::string GetFactionDiplomacyStateProposal(std::string civilization_name, std::string faction_name, std::string second_civilization_name, std::string second_faction_name)
{
	...
	if (civilization && second_civilization) {
		...
		int second_faction = CFaction::GetFactionIndex(second_faction_name);
		if (faction != -1 && second_faction != -1) {
			return GetDiplomacyStateNameById(GrandStrategyGame.Factions[civilization->GetIndex()][faction]->GetDiplomacyStateProposal(GrandStrategyGame.Factions[second_civilization->GetIndex()][second_faction]));
		}
	}
...
void SetFactionTier(std::string civilization_name, std::string faction_name, std::string faction_tier_name)
{
	...
	if (civilization) {
		int faction = CFaction::GetFactionIndex(faction_name);
		if (faction != -1) {
			GrandStrategyGame.Factions[civilization->GetIndex()][faction]->FactionTier = faction_tier_id;
		}
	}
}
...
std::string GetFactionTier(std::string civilization_name, std::string faction_name)
{
	...
	if (civilization) {
		int faction = CFaction::GetFactionIndex(faction_name);
		if (faction != -1) {
			return GetFactionTierNameById(GrandStrategyGame.Factions[civilization->GetIndex()][faction]->FactionTier);
		}
	}
...
{
	if (!type.BoolFlag[BUILDING_INDEX].value && type.DefaultStat.Variables[DEMAND_INDEX].Value > 0 && type.GetClass() != nullptr && type.GetClass()->Ident != "caravan") {
		return true;
	}
	return false;
}
...
{
	if (IsGrandStrategyUnit(type) && type.GetClass() != nullptr && type.GetClass()->Ident != "worker") {
		return true;
	}
	return false;
}
...
void SetFactionMinister(std::string civilization_name, std::string faction_name, std::string title_name, std::string hero_full_name)
{
	...
	int faction = -1;
	if (civilization) {
		faction = CFaction::GetFactionIndex(faction_name);
	}
	...
	GrandStrategyGame.Factions[civilization->GetIndex()][faction]->SetMinister(title, TransliterateText(hero_full_name));
}
