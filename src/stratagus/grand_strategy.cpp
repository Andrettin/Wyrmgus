...
void CGrandStrategyProvince::SetOwner(int civilization_id, int faction_id)
{
	...
	for (const CUnitType *unit_type : CUnitType::UnitTypes) { //change the province's military score to be appropriate for the new faction's technologies
...
void CGrandStrategyProvince::SetSettlementBuilding(int building_id, bool has_settlement_building)
{
	...
	if (CUnitType::UnitTypes[building_id]->GetCivilization() != nullptr) {
	...
	if (CUnitType::UnitTypes[building_id]->Class != nullptr && CUnitType::UnitTypes[building_id]->Class->Ident == "stronghold") { //increase the military score of the province, if this building is a stronghold
...
//	fprintf(stderr, "Setting the quantity of unit type \"%s\" to %d in the %s province.\n", CUnitType::UnitTypes[unit_type_id]->Ident.c_str(), quantity, this->Name.c_str());
	...
	if (CUnitType::UnitTypes[unit_type_id]->Class != nullptr && CUnitType::UnitTypes[unit_type_id]->Class->Ident == "worker") {
		...
		if (CUnitType::UnitTypes[unit_type_id]->GetCivilization() != nullptr) {
			militia_unit_type = CCivilization::GetCivilizationClassUnitType(CUnitType::UnitTypes[unit_type_id]->GetCivilization(), UnitClass::Get("militia"));
		}
		if (militia_unit_type != -1) {
...
std::string CGrandStrategyFaction::GetFullName()
{
	if (CFaction::Get(this->Faction)->Type == FactionTypeTribe) {
		return CFaction::Get(this->Faction)->Name;
...
void SetProvinceOwner(std::string province_name, std::string civilization_name, std::string faction_name)
{
	...
	int faction_id = CFaction::GetIndex(faction_name);
...
void AddProvinceClaim(std::string province_name, std::string civilization_name, std::string faction_name)
{
	...
	if (province_id != -1 && GrandStrategyGame.Provinces[province_id]) {
		CCivilization *civilization = CCivilization::Get(civilization_name);
		if (civilization) {
			int faction = CFaction::GetIndex(faction_name);
			if (faction != -1) {
...
void SetFactionDiplomacyStateProposal(std::string civilization_name, std::string faction_name, std::string second_civilization_name, std::string second_faction_name, std::string diplomacy_state_name)
{
	...
	if (civilization && second_civilization) {
		int faction = CFaction::GetIndex(faction_name);
		int second_faction = CFaction::GetIndex(second_faction_name);
...
void SetFactionMinister(std::string civilization_name, std::string faction_name, std::string title_name, std::string hero_full_name)
{
	...
	int faction = -1;
	if (civilization) {
		faction = CFaction::GetIndex(faction_name);
	}
