...
bool CUniqueItem::CanDrop() const
{
	...
	if (!IsNetworkGame()) {
		for (stratagus::character *character : stratagus::character::get_all()) {
...
void CPersistentItem::ProcessConfigData(const CConfigData *config_data)
{
	...
	if (is_equipped && this->Owner && stratagus::get_item_class_slot(this->Type->get_item_class()) != stratagus::item_slot::none) {
		this->Owner->EquippedItems[static_cast<int>(stratagus::get_item_class_slot(this->Type->get_item_class()))].push_back(this);
	}
}
