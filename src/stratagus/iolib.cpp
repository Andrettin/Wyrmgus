...
static void LibraryFileName(const char *file, std::array<char, PATH_MAX> &buffer)
{
	...
	if (!GameName.empty()) {
		snprintf(buffer.data(), PATH_MAX, "%s/%s/%s", path::to_string(parameters::get()->GetUserDirectory()).c_str(), GameName.c_str(), file);
		if (FindFileWithExtension(buffer)) {
			return;
		}
	}

	snprintf(buffer.data(), PATH_MAX, "%s/%s", path::to_string(parameters::get()->GetUserDirectory()).c_str(), file);
	if (FindFileWithExtension(buffer)) {
		return;
	}
