...
//			return unit.Type->MapSound.Acknowledgement.Sound;
			if (unit.Type->MapSound.Acknowledgement.Sound) {
				...
				if (unit.Player->Race != -1 && CCivilization::Get(unit.Player->Race) != civilization && unit.Player->GetFaction() != nullptr && unit.Type->GetIndex() == CFaction::GetFactionClassUnitType(unit.Player->GetFaction(), unit.Type->GetClass())) {
					civilization = CCivilization::Get(unit.Player->Race);
				}
