	...
	if (unit_type) {
		...
		for (size_t var = 0; var < UnitTypeVar.GetNumberVariable(); ++var) {
			if (
				...
				|| var == ETHEREALVISION_INDEX || var == SPEEDBONUS_INDEX || var == RAIL_SPEED_BONUS_INDEX || var == SUPPLY_INDEX || var == TIMEEFFICIENCYBONUS_INDEX || var == RESEARCHSPEEDBONUS_INDEX || var == GARRISONEDRANGEBONUS_INDEX || CAPTURABLE_INDEX)
			) {
				continue;
			}
